...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const 
{
	...
	//separate them using projection
	...
	tree.OperateOnContents([&](std::list<QuadTreeEntry<GameObject*>>& data)
		{
			...
			for (auto i = data.begin(); i != data.end(); ++i)
			{
				...
				{
					//is this pair of items already in the collion set?
...
void PhysicsSystem::NarrowPhase() 
{
	for (std::set<CollisionDetection::CollisionInfo>::iterator i = broadphaseCollisions.begin(); i != broadphaseCollisions.end(); ++i)
	{
		...
		{
			string aName = info.a->GetName();
			string bName = info.b->GetName();
			info.framesLeft = numCollisionFrames;
		
			//dont want impulse for these items
			if ((aName == "goose" && (bName == "water" || bName == "snitch" || bName == "apple")) || (bName == "goose" && (aName == "water" || aName == "snitch" || aName == "apple")))
			{

			}

			else if ((aName == "water" && (bName == "apple" || bName == "snitch")) || (bName == "water" && (aName == "apple" || aName == "snitch")))
			{
